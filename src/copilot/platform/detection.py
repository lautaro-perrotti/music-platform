...
def detect_ableton(port: int = 9877, *, include_start_menu: bool = True) -> AbletonDetection:
    ...
    if not exe_path:
        discovered_command = shutil.which("ableton")
        if discovered_command:
            exe_path = discovered_command
            evidence.append(f"PATH executable {discovered_command}")
...
def prefs_search_roots() -> list[Path]:
    ...
    if platform.system() == "Linux":
        roots.append(home / ".config" / "Ableton")
    return roots
...
def _candidate_install_roots() -> list[Path]:
    ...
    if os.name == "nt":
        return [
            Path(os.environ.get("ProgramFiles", r"C:\Program Files")) / "Ableton",
            Path(os.environ.get("ProgramFiles(x86)", r"C:\Program Files (x86)")) / "Ableton",
            Path(os.environ.get("ProgramData", r"C:\ProgramData")) / "Ableton",
...
def _first_live_exe(root: Path) -> Path | None:
    ...
    if not root.exists():
        return None
    patterns = ("**/Ableton Live *.exe", "**/Ableton Live", "**/ableton")
    matches = [item for pattern in patterns for item in root.glob(pattern) if item.is_file()]
