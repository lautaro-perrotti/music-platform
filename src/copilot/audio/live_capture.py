...
def install_audio_tap_device() -> dict[str, str | bool]:
    ...
    dest = USER_LIBRARY_TAP
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(DEVICE_SOURCE, dest)
    capture_dir()
    return {
...
def _find_tap_uri(daw: AbletonTcpAdapter) -> str | None:
    searched = daw.search_browser(TAP_NAME, "audio_effects")
    for item in searched.get("results") or []:
        name = str(item.get("name") or "")
        if TAP_NAME.lower() in name.lower() and item.get("is_loadable"):
            uri = item.get("uri")
            if uri:
                return str(uri)
    for path in (
        ...
        ["audio_effects", "User Library"],
        ["audio_effects", "User Library", "Max Audio Effect"],
    ):
        listing = daw.browse_path(path)
