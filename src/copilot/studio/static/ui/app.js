...
function syncShellContext() {
  ...
  topBar.setAttribute('jobs', jobs.length ? `${jobs.length} jobs` : 'No jobs');
  topBar.paint?.();
...
async function showProjects() {
  ...
  const rows = data.projects.map(project => `<div class="row"><div class="name">${esc(project.name)}<small>${esc(project.project_id)}</small></div><div class="table-muted">${project.project_id === state.project?.project_id ? 'Active project' : 'No active version'}</div><div class="table-muted">—</div><div class="table-muted">—</div><div class="table-muted">Read-only</div><div><button class="app-native-button" data-action="open-project" data-project-id="${project.project_id}">Open</button></div></div>`).join('');
  setMain(`<section class="app-page"><div class="page-heading"><div><div class="eyebrow">Workspace</div><h1>Projects <span class="table-muted">${data.projects.length}</span></h1></div><div class="page-toolbar"><input class="search-input" style="height:32px;width:220px;padding:0 10px;border:1px solid var(--ms-line);border-radius:var(--ms-radius-control);background:var(--ms-raised);color:var(--ms-text)" placeholder="Search projects" aria-label="Search projects"><button class="app-native-button primary" data-action="go-create">New project</button></div></div><div class="source-tabs"><button class="active">All projects</button><button>Recent</button><button>Archived</button></div><div class="project-table"><div class="row head"><div>Name</div><div>Version</div><div>Length</div><div>Refs</div><div>Live</div><div></div></div>${rows || `<div class="empty-table"><ms-empty-state title="No projects yet" description="Create a project to start a real production session." action="New project"></ms-empty-state></div>`}</div><p class="table-muted" style="margin-top:14px">Projects are durable workspaces. Original references remain read-only.</p></section>`); }
